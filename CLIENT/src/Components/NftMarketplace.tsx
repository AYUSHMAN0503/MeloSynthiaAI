import React from 'react'
import "./NFT.css"
import Bg from "./../assets/musical.jpg"
import { Link } from 'react-router-dom'
import Animatedpage from './Animatedpage'
const NftMarketplace : React.FC = () => {
  const scrollToTop = () => {
    window.scrollTo({ top: 0, left: 0, behavior: 'smooth' });
  };
  const scrollToHalf = () => {
    const windowHeight = window.innerHeight;
    const halfWindowHeight = windowHeight / 1.1;
    window.scrollTo({ top: halfWindowHeight, left: 0, behavior: 'smooth' });
  };
  return (
    <Animatedpage>
    <div>

        <section id="home" className="f z gd xe ye ze mt-24 kg">
        <span className="e _">
              
            </span>
          <div className="e h g _ ib pb" />
          <div className="a">
            <div className="ea za xc yc">
              <div className="pb nf _k/2">
                <div className="ra gc tk">
                  <h1 className="ia lh ph qh vh lk tl om 2xl:ud-text-[50px] sm:text-[46px]">
                   Lorem ipsum dolor sit, amet consectetur adipisicing elit. Assumenda, sit?
                  </h1>
                  <p className="qa hh oh rh uh kk">
                 Lorem ipsum dolor sit amet consectetur adipisicing elit. Quae debitis esse aliquid officiis. Qui, eveniet. Ullam excepturi dolor possimus a.
                  </p>
                  <div className="flex flex-wrap items-center">
                    <a onClick={scrollToHalf} href="#"className="pa ka _a yc _c ld pd yd he sf vf eh nh vh ii aj">
                     Buy NFT
                    </a>
                    <Link onClick={scrollToTop} to="/SellNft" className="pa ka _a yc _c ld pd yd he sf vf eh nh vh ii aj">
                     Sell NFT
                    </Link>
                    
                  </div>
                </div>
              </div>
              
            </div>
          </div>
          <div>
            
         
          </div>
        </section>
        <section className="ug">
          <div className="a">
            <div className="ra td vd _d">
              <div className="ea za">
                <div className="pb nf">
                  <h2 className="ka mh ph vh">
                    Top Artists
                  </h2>
                </div>
              </div>
            </div>
            <div className="f">
              <div className="artists-slider ea za xc">
                <div className="pb nf hk/2 gm/3 2xl:ud-w-1/4">
                  <div className="gradient-bg ka nd if">
                    <div className="za yc nd fe jf">
                      <div className="pa jb pb hc gd kd">
                        <img src="src/images/artists/image-01.png" alt="artists" className="ib pb af bf" />
                      </div>
                      <div className="pb">
                        <h3 className="id hh nh vh">
                          @Devid_Miller
                        </h3>
                        <span className="eh nh xh">
                          14.55 ETH
                        </span>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="pb nf hk/2 gm/3 2xl:ud-w-1/4">
                  <div className="gradient-bg ka nd if">
                    <div className="za yc nd fe jf">
                      <div className="pa jb pb hc gd kd">
                        <img src="src/images/artists/image-01.png" alt="artists" className="ib pb af bf" />
                      </div>
                      <div className="pb">
                        <h3 className="id hh nh vh">
                          @Devid_Miller
                        </h3>
                        <span className="eh nh xh">
                          14.55 ETH
                        </span>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="pb nf hk/2 gm/3 2xl:ud-w-1/4">
                  <div className="gradient-bg ka nd if">
                    <div className="za yc nd fe jf">
                      <div className="pa jb pb hc gd kd">
                        <img src="src/images/artists/image-02.png" alt="artists" className="ib pb af bf" />
                      </div>
                      <div className="pb">
                        <h3 className="id hh nh vh">
                          @Elon_Mask
                        </h3>
                        <span className="eh nh xh">
                          12.25 ETH
                        </span>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="pb nf hk/2 gm/3 2xl:ud-w-1/4">
                  <div className="gradient-bg ka nd if">
                    <div className="za yc nd fe jf">
                      <div className="pa jb pb hc gd kd">
                        <img src="src/images/artists/image-03.png" alt="artists" className="ib pb af bf" />
                      </div>
                      <div className="pb">
                        <h3 className="id hh nh vh">
                          @Hiliya_Farah
                        </h3>
                        <span className="eh nh xh">
                          9.89 ETH
                        </span>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="pb nf hk/2 gm/3 2xl:ud-w-1/4">
                  <div className="gradient-bg ka nd if">
                    <div className="za yc nd fe jf">
                      <div className="pa jb pb hc gd kd">
                        <img src="src/images/artists/image-04.png" alt="artists" className="ib pb af bf" />
                      </div>
                      <div className="pb">
                        <h3 className="id hh nh vh">
                          @Wilium_Dev
                        </h3>
                        <span className="eh nh xh">
                          2.09 ETH
                        </span>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </section>
        <section className="wg">
          <div className="a">
            <div className="ra td vd _d">
              <div className="ad qj">
                <h2 className="ia mh ph sh vh">
                  Live Auctions
                </h2>
                <a href="javascript:void(0)" className="ka _a yc _c ld he uf wf eh nh vh ii aj">
                  View All
                </a>
              </div>
            </div>
            <div className="ea za xc">
              <div className="pb nf hk/2 _k/3 2xl:ud-w-1/4">
                <div className="ja nd qd wd fe hf">
                  <div className="f ka gd kd">
                    <img src="src/images/auctions/image-01.svg" alt="auctions" className="pb" />
                  </div>
                  <div>
                    <h3>
                      <a href="javascript:void(0)" className="sa ya hh nh vh cj">
                        22-Bit Digital #551
                      </a>
                    </h3>
                    <div className="ka za yc ad">
                      <div className="pb">
                        <div className="za yc">
                          <div className="ta hb pb fc ld">
                            <img src="src/images/auctions/creator-01.png" alt="creator" className="ib pb af bf" />
                          </div>
                          <div className="pb">
                            <h4 className="kh nh vh">
                              @Devid_Mill...
                              <span className="xa kh oh uh">
                                creator
                              </span>
                            </h4>
                          </div>
                        </div>
                      </div>
                      <div className="pb">
                        <h5 className="dh kh nh vh">
                          5.49 ETH
                          <span className="xa kh oh uh">
                            Current Bit
                          </span>
                        </h5>
                      </div>
                    </div>
                    <div className="za yc _c ld ee se kf eh nh vh">
                      05 : 15 : 07 : 45
                    </div>
                  </div>
                </div>
              </div>
              <div className="pb nf hk/2 _k/3 2xl:ud-w-1/4">
                <div className="ja nd qd wd fe hf">
                  <div className="f ka gd kd">
                    <img src="src/images/auctions/image-02.svg" alt="auctions" className="pb" />
                   
                  </div>
                  <div>
                    <h3>
                      <a href="javascript:void(0)" className="sa ya hh nh vh cj">
                        25-Bit Digital #729
                      </a>
                    </h3>
                    <div className="ka za yc ad">
                      <div className="pb">
                        <div className="za yc">
                          <div className="ta hb pb fc ld">
                            <img src="src/images/auctions/creator-02.png" alt="creator" className="ib pb af bf" />
                          </div>
                          <div className="pb">
                            <h4 className="kh nh vh">
                              @Devid_Meth...
                              <span className="xa kh oh uh">
                                creator
                              </span>
                            </h4>
                          </div>
                        </div>
                      </div>
                      <div className="pb">
                        <h5 className="dh kh nh vh">
                          1.58 ETH
                          <span className="xa kh oh uh">
                            Current Bit
                          </span>
                        </h5>
                      </div>
                    </div>
                    <div className="za yc _c ld ee se kf eh nh vh">
                      06 : 15 : 07 : 55
                    </div>
                  </div>
                </div>
              </div>
              <div className="pb nf hk/2 _k/3 2xl:ud-w-1/4">
                <div className="ja nd qd wd fe hf">
                  <div className="f ka gd kd">
                    <img src="src/images/auctions/image-03.svg" alt="auctions" className="pb" />
                  
                  </div>
                  <div>
                    <h3>
                      <a href="javascript:void(0)" className="sa ya hh nh vh cj">
                        27-Bit Digital #395
                      </a>
                    </h3>
                    <div className="ka za yc ad">
                      <div className="pb">
                        <div className="za yc">
                          <div className="ta hb pb fc ld">
                            <img src="src/images/auctions/creator-03.png" alt="creator" className="ib pb af bf" />
                          </div>
                          <div className="pb">
                            <h4 className="kh nh vh">
                              @Jemmy_Fra...
                              <span className="xa kh oh uh">
                                creator
                              </span>
                            </h4>
                          </div>
                        </div>
                      </div>
                      <div className="pb">
                        <h5 className="dh kh nh vh">
                          3.25 ETH
                          <span className="xa kh oh uh">
                            Current Bit
                          </span>
                        </h5>
                      </div>
                    </div>
                    <div className="za yc _c ld ee se kf eh nh vh">
                      02 : 15 : 08 : 44
                    </div>
                  </div>
                </div>
              </div>
              <div className="pb nf hk/2 _k/3 2xl:ud-w-1/4">
                <div className="ja nd qd wd fe hf">
                  <div className="f ka gd kd">
                    <img src="src/images/auctions/image-04.svg" alt="auctions" className="pb" />
                   
                  </div>
                  <div>
                    <h3>
                      <a href="javascript:void(0)" className="sa ya hh nh vh cj">
                        35-Bit Digital #439
                      </a>
                    </h3>
                    <div className="ka za yc ad">
                      <div className="pb">
                        <div className="za yc">
                          <div className="ta hb pb fc ld">
                            <img src="src/images/auctions/creator-04.png" alt="creator" className="ib pb af bf" />
                          </div>
                          <div className="pb">
                            <h4 className="kh nh vh">
                              @Miliya_Cho...
                              <span className="xa kh oh uh">
                                creator
                              </span>
                            </h4>
                          </div>
                        </div>
                      </div>
                      <div className="pb">
                        <h5 className="dh kh nh vh">
                          1.57 ETH
                          <span className="xa kh oh uh">
                            Current Bit
                          </span>
                        </h5>
                      </div>
                    </div>
                    <div className="za yc _c ld ee se kf eh nh vh">
                      07 : 15 : 03 : 12
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </section>
        <section className="rg">
          <div className="a">
            <div className="ra td vd _d">
              <div className="ad qj">
                <h2 className="ia mh ph sh vh">
                  Today's Picks
                </h2>
                <a href="javascript:void(0)" className="ka _a yc _c ld he uf wf eh nh vh ii aj">
                  View All
                </a>
              </div>
            </div>
            <div className="ea za xc">
              <div className="pb nf hk/2 _k/3 2xl:ud-w-1/4">
                <div className="ja nd qd wd fe hf">
                  <div className="f ka gd kd">
                    <img src="src/images/picks/image-01.svg" alt="auctions" className="pb" />
                   
                  </div>
                  <div>
                    <h3>
                      <a href="javascript:void(0)" className="sa ya hh nh vh cj">
                        3d abstract illustration
                      </a>
                    </h3>
                    <div className="oa za yc ad">
                      <div className="pb">
                        <div className="za yc">
                          <div className="ta hb pb fc ld">
                            <img src="src/images/picks/creator-01.png" alt="creator" className="ib pb af bf" />
                          </div>
                          <div className="pb">
                            <h4 className="kh nh vh">
                              @Devid_Mill...
                              <span className="xa kh oh uh">
                                creator
                              </span>
                            </h4>
                          </div>
                        </div>
                      </div>
                      <div className="pb">
                        <h5 className="dh kh nh vh">
                          5.49 ETH
                          <span className="xa kh oh uh">
                            Current Bit
                          </span>
                        </h5>
                      </div>
                    </div>
                    <div className="za yc ad sd wd ug">
                      <a href="javascript:void(0)" className="za yc _c ld he sf nf fh nh vh ii aj xj">
                        Place Bid
                      </a>
                      <a href="javascript:void(0)" className="za yc _c ld sf nf fh nh vh cj xj">
                        <span className="vg">
                          <svg width={18} height={18} viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9 4.5V6.75L12 3.75L9 0.75V3C7.4087 3 5.88258 3.63214 4.75736 4.75736C3.63214 5.88258 3 7.4087 3 9C3 10.1775 3.345 11.2725 3.93 12.195L5.025 11.1C4.6875 10.4775 4.5 9.75 4.5 9C4.5 7.80653 4.97411 6.66193 5.81802 5.81802C6.66193 4.97411 7.80652 4.5 9 4.5ZM14.07 5.805L12.975 6.9C13.305 7.53 13.5 8.25 13.5 9C13.5 10.1935 13.0259 11.3381 12.182 12.182C11.3381 13.0259 10.1935 13.5 9 13.5V11.25L6 14.25L9 17.25V15C10.5913 15 12.1174 14.3679 13.2426 13.2426C14.3679 12.1174 15 10.5913 15 9C15 7.8225 14.655 6.7275 14.07 5.805Z" fill="currentColor" />
                          </svg>
                        </span>
                        View History
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div className="pb nf hk/2 _k/3 2xl:ud-w-1/4">
                <div className="ja nd qd wd fe hf">
                  <div className="f ka gd kd">
                    <img src="src/images/picks/image-02.svg" alt="auctions" className="pb" />
                   
                  </div>
                  <div>
                    <h3>
                      <a href="javascript:void(0)" className="sa ya hh nh vh cj">
                        3d abstract illustration
                      </a>
                    </h3>
                    <div className="oa za yc ad">
                      <div className="pb">
                        <div className="za yc">
                          <div className="ta hb pb fc ld">
                            <img src="src/images/picks/creator-02.png" alt="creator" className="ib pb af bf" />
                          </div>
                          <div className="pb">
                            <h4 className="kh nh vh">
                              @Wilium_de...
                              <span className="xa kh oh uh">
                                creator
                              </span>
                            </h4>
                          </div>
                        </div>
                      </div>
                      <div className="pb">
                        <h5 className="dh kh nh vh">
                          2.85 ETH
                          <span className="xa kh oh uh">
                            Current Bit
                          </span>
                        </h5>
                      </div>
                    </div>
                    <div className="za yc ad sd wd ug">
                      <a href="javascript:void(0)" className="za yc _c ld he sf nf fh nh vh ii aj xj">
                        Place Bid
                      </a>
                      <a href="javascript:void(0)" className="za yc _c ld sf nf fh nh vh cj xj">
                        <span className="vg">
                          <svg width={18} height={18} viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9 4.5V6.75L12 3.75L9 0.75V3C7.4087 3 5.88258 3.63214 4.75736 4.75736C3.63214 5.88258 3 7.4087 3 9C3 10.1775 3.345 11.2725 3.93 12.195L5.025 11.1C4.6875 10.4775 4.5 9.75 4.5 9C4.5 7.80653 4.97411 6.66193 5.81802 5.81802C6.66193 4.97411 7.80652 4.5 9 4.5ZM14.07 5.805L12.975 6.9C13.305 7.53 13.5 8.25 13.5 9C13.5 10.1935 13.0259 11.3381 12.182 12.182C11.3381 13.0259 10.1935 13.5 9 13.5V11.25L6 14.25L9 17.25V15C10.5913 15 12.1174 14.3679 13.2426 13.2426C14.3679 12.1174 15 10.5913 15 9C15 7.8225 14.655 6.7275 14.07 5.805Z" fill="currentColor" />
                          </svg>
                        </span>
                        View History
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div className="pb nf hk/2 _k/3 2xl:ud-w-1/4">
                <div className="ja nd qd wd fe hf">
                  <div className="f ka gd kd">
                    <img src="src/images/picks/image-03.svg" alt="auctions" className="pb" />
                   
                  </div>
                  <div>
                    <h3>
                      <a href="javascript:void(0)" className="sa ya hh nh vh cj">
                        3d abstract illustration
                      </a>
                    </h3>
                    <div className="oa za yc ad">
                      <div className="pb">
                        <div className="za yc">
                          <div className="ta hb pb fc ld">
                            <img src="src/images/picks/creator-03.png" alt="creator" className="ib pb af bf" />
                          </div>
                          <div className="pb">
                            <h4 className="kh nh vh">
                              @Nicolo_Tes...
                              <span className="xa kh oh uh">
                                creator
                              </span>
                            </h4>
                          </div>
                        </div>
                      </div>
                      <div className="pb">
                        <h5 className="dh kh nh vh">
                          7.82 ETH
                          <span className="xa kh oh uh">
                            Current Bit
                          </span>
                        </h5>
                      </div>
                    </div>
                    <div className="za yc ad sd wd ug">
                      <a href="javascript:void(0)" className="za yc _c ld he sf nf fh nh vh ii aj xj">
                        Place Bid
                      </a>
                      <a href="javascript:void(0)" className="za yc _c ld sf nf fh nh vh cj xj">
                        <span className="vg">
                          <svg width={18} height={18} viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9 4.5V6.75L12 3.75L9 0.75V3C7.4087 3 5.88258 3.63214 4.75736 4.75736C3.63214 5.88258 3 7.4087 3 9C3 10.1775 3.345 11.2725 3.93 12.195L5.025 11.1C4.6875 10.4775 4.5 9.75 4.5 9C4.5 7.80653 4.97411 6.66193 5.81802 5.81802C6.66193 4.97411 7.80652 4.5 9 4.5ZM14.07 5.805L12.975 6.9C13.305 7.53 13.5 8.25 13.5 9C13.5 10.1935 13.0259 11.3381 12.182 12.182C11.3381 13.0259 10.1935 13.5 9 13.5V11.25L6 14.25L9 17.25V15C10.5913 15 12.1174 14.3679 13.2426 13.2426C14.3679 12.1174 15 10.5913 15 9C15 7.8225 14.655 6.7275 14.07 5.805Z" fill="currentColor" />
                          </svg>
                        </span>
                        View History
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div className="pb nf hk/2 _k/3 2xl:ud-w-1/4">
                <div className="ja nd qd wd fe hf">
                  <div className="f ka gd kd">
                    <img src="src/images/picks/image-04.svg" alt="auctions" className="pb" />
                    
                  </div>
                  <div>
                    <h3>
                      <a href="javascript:void(0)" className="sa ya hh nh vh cj">
                        3d abstract illustration
                      </a>
                    </h3>
                    <div className="oa za yc ad">
                      <div className="pb">
                        <div className="za yc">
                          <div className="ta hb pb fc ld">
                            <img src="src/images/picks/creator-04.png" alt="creator" className="ib pb af bf" />
                          </div>
                          <div className="pb">
                            <h4 className="kh nh vh">
                              @Liza_Auro...
                              <span className="xa kh oh uh">
                                creator
                              </span>
                            </h4>
                          </div>
                        </div>
                      </div>
                      <div className="pb">
                        <h5 className="dh kh nh vh">
                          0.25 ETH
                          <span className="xa kh oh uh">
                            Current Bit
                          </span>
                        </h5>
                      </div>
                    </div>
                    <div className="za yc ad sd wd ug">
                      <a href="javascript:void(0)" className="za yc _c ld he sf nf fh nh vh ii aj xj">
                        Place Bid
                      </a>
                      <a href="javascript:void(0)" className="za yc _c ld sf nf fh nh vh cj xj">
                        <span className="vg">
                          <svg width={18} height={18} viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9 4.5V6.75L12 3.75L9 0.75V3C7.4087 3 5.88258 3.63214 4.75736 4.75736C3.63214 5.88258 3 7.4087 3 9C3 10.1775 3.345 11.2725 3.93 12.195L5.025 11.1C4.6875 10.4775 4.5 9.75 4.5 9C4.5 7.80653 4.97411 6.66193 5.81802 5.81802C6.66193 4.97411 7.80652 4.5 9 4.5ZM14.07 5.805L12.975 6.9C13.305 7.53 13.5 8.25 13.5 9C13.5 10.1935 13.0259 11.3381 12.182 12.182C11.3381 13.0259 10.1935 13.5 9 13.5V11.25L6 14.25L9 17.25V15C10.5913 15 12.1174 14.3679 13.2426 13.2426C14.3679 12.1174 15 10.5913 15 9C15 7.8225 14.655 6.7275 14.07 5.805Z" fill="currentColor" />
                          </svg>
                        </span>
                        View History
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div className="pb nf hk/2 _k/3 2xl:ud-w-1/4">
                <div className="ja nd qd wd fe hf">
                  <div className="f ka gd kd">
                    <img src="src/images/picks/image-05.svg" alt="auctions" className="pb" />
                   
                  </div>
                  <div>
                    <h3>
                      <a href="javascript:void(0)" className="sa ya hh nh vh cj">
                        3d abstract illustration
                      </a>
                    </h3>
                    <div className="oa za yc ad">
                      <div className="pb">
                        <div className="za yc">
                          <div className="ta hb pb fc ld">
                            <img src="src/images/picks/creator-05.png" alt="creator" className="ib pb af bf" />
                          </div>
                          <div className="pb">
                            <h4 className="kh nh vh">
                              @Lathium_Lui...
                              <span className="xa kh oh uh">
                                creator
                              </span>
                            </h4>
                          </div>
                        </div>
                      </div>
                      <div className="pb">
                        <h5 className="dh kh nh vh">
                          3.24 ETH
                          <span className="xa kh oh uh">
                            Current Bit
                          </span>
                        </h5>
                      </div>
                    </div>
                    <div className="za yc ad sd wd ug">
                      <a href="javascript:void(0)" className="za yc _c ld he sf nf fh nh vh ii aj xj">
                        Place Bid
                      </a>
                      <a href="javascript:void(0)" className="za yc _c ld sf nf fh nh vh cj xj">
                        <span className="vg">
                          <svg width={18} height={18} viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9 4.5V6.75L12 3.75L9 0.75V3C7.4087 3 5.88258 3.63214 4.75736 4.75736C3.63214 5.88258 3 7.4087 3 9C3 10.1775 3.345 11.2725 3.93 12.195L5.025 11.1C4.6875 10.4775 4.5 9.75 4.5 9C4.5 7.80653 4.97411 6.66193 5.81802 5.81802C6.66193 4.97411 7.80652 4.5 9 4.5ZM14.07 5.805L12.975 6.9C13.305 7.53 13.5 8.25 13.5 9C13.5 10.1935 13.0259 11.3381 12.182 12.182C11.3381 13.0259 10.1935 13.5 9 13.5V11.25L6 14.25L9 17.25V15C10.5913 15 12.1174 14.3679 13.2426 13.2426C14.3679 12.1174 15 10.5913 15 9C15 7.8225 14.655 6.7275 14.07 5.805Z" fill="currentColor" />
                          </svg>
                        </span>
                        View History
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div className="pb nf hk/2 _k/3 2xl:ud-w-1/4">
                <div className="ja nd qd wd fe hf">
                  <div className="f ka gd kd">
                    <img src="src/images/picks/image-06.svg" alt="auctions" className="pb" />
                   
                  </div>
                  <div>
                    <h3>
                      <a href="javascript:void(0)" className="sa ya hh nh vh cj">
                        3d abstract illustration
                      </a>
                    </h3>
                    <div className="oa za yc ad">
                      <div className="pb">
                        <div className="za yc">
                          <div className="ta hb pb fc ld">
                            <img src="src/images/picks/creator-06.png" alt="creator" className="ib pb af bf" />
                          </div>
                          <div className="pb">
                            <h4 className="kh nh vh">
                              @Marko...
                              <span className="xa kh oh uh">
                                creator
                              </span>
                            </h4>
                          </div>
                        </div>
                      </div>
                      <div className="pb">
                        <h5 className="dh kh nh vh">
                          4.55 ETH
                          <span className="xa kh oh uh">
                            Current Bit
                          </span>
                        </h5>
                      </div>
                    </div>
                    <div className="za yc ad sd wd ug">
                      <a href="javascript:void(0)" className="za yc _c ld he sf nf fh nh vh ii aj xj">
                        Place Bid
                      </a>
                      <a href="javascript:void(0)" className="za yc _c ld sf nf fh nh vh cj xj">
                        <span className="vg">
                          <svg width={18} height={18} viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9 4.5V6.75L12 3.75L9 0.75V3C7.4087 3 5.88258 3.63214 4.75736 4.75736C3.63214 5.88258 3 7.4087 3 9C3 10.1775 3.345 11.2725 3.93 12.195L5.025 11.1C4.6875 10.4775 4.5 9.75 4.5 9C4.5 7.80653 4.97411 6.66193 5.81802 5.81802C6.66193 4.97411 7.80652 4.5 9 4.5ZM14.07 5.805L12.975 6.9C13.305 7.53 13.5 8.25 13.5 9C13.5 10.1935 13.0259 11.3381 12.182 12.182C11.3381 13.0259 10.1935 13.5 9 13.5V11.25L6 14.25L9 17.25V15C10.5913 15 12.1174 14.3679 13.2426 13.2426C14.3679 12.1174 15 10.5913 15 9C15 7.8225 14.655 6.7275 14.07 5.805Z" fill="currentColor" />
                          </svg>
                        </span>
                        View History
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div className="pb nf hk/2 _k/3 2xl:ud-w-1/4">
                <div className="ja nd qd wd fe hf">
                  <div className="f ka gd kd">
                    <img src="src/images/picks/image-07.svg" alt="auctions" className="pb" />
                   
                  </div>
                  <div>
                    <h3>
                      <a href="javascript:void(0)" className="sa ya hh nh vh cj">
                        3d abstract illustration
                      </a>
                    </h3>
                    <div className="oa za yc ad">
                      <div className="pb">
                        <div className="za yc">
                          <div className="ta hb pb fc ld">
                            <img src="src/images/picks/creator-07.png" alt="creator" className="ib pb af bf" />
                          </div>
                          <div className="pb">
                            <h4 className="kh nh vh">
                              @Andrio_Hev...
                              <span className="xa kh oh uh">
                                creator
                              </span>
                            </h4>
                          </div>
                        </div>
                      </div>
                      <div className="pb">
                        <h5 className="dh kh nh vh">
                          0.89 ETH
                          <span className="xa kh oh uh">
                            Current Bit
                          </span>
                        </h5>
                      </div>
                    </div>
                    <div className="za yc ad sd wd ug">
                      <a href="javascript:void(0)" className="za yc _c ld he sf nf fh nh vh ii aj xj">
                        Place Bid
                      </a>
                      <a href="javascript:void(0)" className="za yc _c ld sf nf fh nh vh cj xj">
                        <span className="vg">
                          <svg width={18} height={18} viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9 4.5V6.75L12 3.75L9 0.75V3C7.4087 3 5.88258 3.63214 4.75736 4.75736C3.63214 5.88258 3 7.4087 3 9C3 10.1775 3.345 11.2725 3.93 12.195L5.025 11.1C4.6875 10.4775 4.5 9.75 4.5 9C4.5 7.80653 4.97411 6.66193 5.81802 5.81802C6.66193 4.97411 7.80652 4.5 9 4.5ZM14.07 5.805L12.975 6.9C13.305 7.53 13.5 8.25 13.5 9C13.5 10.1935 13.0259 11.3381 12.182 12.182C11.3381 13.0259 10.1935 13.5 9 13.5V11.25L6 14.25L9 17.25V15C10.5913 15 12.1174 14.3679 13.2426 13.2426C14.3679 12.1174 15 10.5913 15 9C15 7.8225 14.655 6.7275 14.07 5.805Z" fill="currentColor" />
                          </svg>
                        </span>
                        View History
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div className="pb nf hk/2 _k/3 2xl:ud-w-1/4">
                <div className="ja nd qd wd fe hf">
                  <div className="f ka gd kd">
                    <img src="src/images/picks/image-08.svg" alt="auctions" className="pb" />
                  
                  </div>
                  <div>
                    <h3>
                      <a href="javascript:void(0)" className="sa ya hh nh vh cj">
                        3d abstract illustration
                      </a>
                    </h3>
                    <div className="oa za yc ad">
                      <div className="pb">
                        <div className="za yc">
                          <div className="ta hb pb fc ld">
                            <img src="src/images/picks/creator-08.png" alt="creator" className="ib pb af bf" />
                          </div>
                          <div className="pb">
                            <h4 className="kh nh vh">
                              @Mariya_Hie...
                              <span className="xa kh oh uh">
                                creator
                              </span>
                            </h4>
                          </div>
                        </div>
                      </div>
                      <div className="pb">
                        <h5 className="dh kh nh vh">
                          1.75 ETH
                          <span className="xa kh oh uh">
                            Current Bit
                          </span>
                        </h5>
                      </div>
                    </div>
                    <div className="za yc ad sd wd ug">
                      <a href="javascript:void(0)" className="za yc _c ld he sf nf fh nh vh ii aj xj">
                        Place Bid
                      </a>
                      <a href="javascript:void(0)" className="za yc _c ld sf nf fh nh vh cj xj">
                        <span className="vg">
                          <svg width={18} height={18} viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9 4.5V6.75L12 3.75L9 0.75V3C7.4087 3 5.88258 3.63214 4.75736 4.75736C3.63214 5.88258 3 7.4087 3 9C3 10.1775 3.345 11.2725 3.93 12.195L5.025 11.1C4.6875 10.4775 4.5 9.75 4.5 9C4.5 7.80653 4.97411 6.66193 5.81802 5.81802C6.66193 4.97411 7.80652 4.5 9 4.5ZM14.07 5.805L12.975 6.9C13.305 7.53 13.5 8.25 13.5 9C13.5 10.1935 13.0259 11.3381 12.182 12.182C11.3381 13.0259 10.1935 13.5 9 13.5V11.25L6 14.25L9 17.25V15C10.5913 15 12.1174 14.3679 13.2426 13.2426C14.3679 12.1174 15 10.5913 15 9C15 7.8225 14.655 6.7275 14.07 5.805Z" fill="currentColor" />
                          </svg>
                        </span>
                        View History
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div className="pb nf">
                <div className="rg ch">
                  <a href="javascript:void(0)" className="_a yc _c ld pd vd sf vf eh nh vh ii wi yi">
                    Load More...
                  </a>
                </div>
              </div>
            </div>
          </div>
        </section>
        <section id="features" className="xg">
          <div className="a">
            <div className="da ha dc ch">
              <h2 className="ia gh ph qh vh ck">
                Core Features
              </h2>
              <p className="hh oh uh">
                Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aliquam a
                lacinia dolor, in pretium nunc. Morbi mollis arcu eget.
              </p>
            </div>
            <div className="ea za xc">
              <div className="pb nf hk/2 _k/3 2xl:ud-w-1/4">
                <div className="ij ja nd qd wd fe ef ii xi zi">
                  <div className="ka za db tb yc _c od le">
                    <svg width={34} height={34} viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M29.75 25.5V26.9167C29.75 27.6681 29.4515 28.3888 28.9201 28.9201C28.3888 29.4515 27.6681 29.75 26.9167 29.75H7.08333C5.51083 29.75 4.25 28.475 4.25 26.9167V7.08333C4.25 6.33189 4.54851 5.61122 5.07986 5.07986C5.61122 4.54851 6.33189 4.25 7.08333 4.25H26.9167C27.6681 4.25 28.3888 4.54851 28.9201 5.07986C29.4515 5.61122 29.75 6.33189 29.75 7.08333V8.5H17C15.4275 8.5 14.1667 9.775 14.1667 11.3333V22.6667C14.1667 23.4181 14.4652 24.1388 14.9965 24.6701C15.5279 25.2015 16.2486 25.5 17 25.5H29.75ZM17 22.6667H31.1667V11.3333H17V22.6667ZM22.6667 19.125C22.1031 19.125 21.5626 18.9011 21.1641 18.5026C20.7656 18.1041 20.5417 17.5636 20.5417 17C20.5417 16.4364 20.7656 15.8959 21.1641 15.4974C21.5626 15.0989 22.1031 14.875 22.6667 14.875C23.2303 14.875 23.7708 15.0989 24.1693 15.4974C24.5678 15.8959 24.7917 16.4364 24.7917 17C24.7917 17.5636 24.5678 18.1041 24.1693 18.5026C23.7708 18.9011 23.2303 19.125 22.6667 19.125Z" fill="#FF766A" />
                    </svg>
                  </div>
                  <h3 className="la ih ph vh gi mj">
                    Set Up Your Wallet
                  </h3>
                  <p className="eh oh uh">
                    Lorem ipsum dolor sit amet consectetur smit.
                  </p>
                </div>
              </div>
              <div className="pb nf hk/2 _k/3 2xl:ud-w-1/4">
                <div className="ij ja nd qd wd fe ef ii xi zi">
                  <div className="ka za db tb yc _c od me">
                    <svg width={34} height={34} viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M4.25 15.5833H15.5833V4.25H4.25V15.5833ZM4.25 29.75H15.5833V18.4167H4.25V29.75ZM18.4167 29.75H29.75V18.4167H18.4167V29.75ZM18.4167 4.25V15.5833H29.75V4.25" fill="#F5B70A" />
                    </svg>
                  </div>
                  <h3 className="la ih ph vh gi mj">
                    Make Your Collection
                  </h3>
                  <p className="eh oh uh">
                    Lorem ipsum dolor sit amet consectetur smit.
                  </p>
                </div>
              </div>
              <div className="pb nf hk/2 _k/3 2xl:ud-w-1/4">
                <div className="ij ja nd qd wd fe ef ii xi zi">
                  <div className="ka za db tb yc _c od ne">
                    <svg width={34} height={34} viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M12.0417 19.125L15.5833 23.375L20.5417 17L26.9167 25.5H7.08333L12.0417 19.125ZM29.75 26.9167V7.08333C29.75 5.51083 28.475 4.25 26.9167 4.25H7.08333C6.33189 4.25 5.61122 4.54851 5.07986 5.07986C4.54851 5.61122 4.25 6.33189 4.25 7.08333V26.9167C4.25 27.6681 4.54851 28.3888 5.07986 28.9201C5.61122 29.4515 6.33189 29.75 7.08333 29.75H26.9167C27.6681 29.75 28.3888 29.4515 28.9201 28.9201C29.4515 28.3888 29.75 27.6681 29.75 26.9167Z" fill="#06DE90" />
                    </svg>
                  </div>
                  <h3 className="la ih ph vh gi mj">
                    Add Your NFTs
                  </h3>
                  <p className="eh oh uh">
                    Lorem ipsum dolor sit amet consectetur smit.
                  </p>
                </div>
              </div>
              <div className="pb nf hk/2 _k/3 2xl:ud-w-1/4">
                <div className="ij ja nd qd wd fe ef ii xi zi">
                  <div className="ka za db tb yc _c od oe">
                    <svg width={34} height={34} viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M7.79168 9.91671C7.22809 9.91671 6.68759 9.69282 6.28907 9.29431C5.89056 8.89579 5.66668 8.35529 5.66668 7.79171C5.66668 7.22812 5.89056 6.68762 6.28907 6.28911C6.68759 5.89059 7.22809 5.66671 7.79168 5.66671C8.35526 5.66671 8.89576 5.89059 9.29428 6.28911C9.69279 6.68762 9.91668 7.22812 9.91668 7.79171C9.91668 8.35529 9.69279 8.89579 9.29428 9.29431C8.89576 9.69282 8.35526 9.91671 7.79168 9.91671ZM30.3308 16.405L17.5808 3.65504C17.0708 3.14504 16.3625 2.83337 15.5833 2.83337H5.66668C4.09418 2.83337 2.83334 4.09421 2.83334 5.66671V15.5834C2.83334 16.3625 3.14501 17.0709 3.66918 17.5809L16.405 30.3309C16.9292 30.8409 17.6375 31.1667 18.4167 31.1667C19.1958 31.1667 19.9042 30.8409 20.4142 30.3309L30.3308 20.4142C30.855 19.9042 31.1667 19.1959 31.1667 18.4167C31.1667 17.6234 30.8408 16.915 30.3308 16.405Z" fill="#5142FC" />
                    </svg>
                  </div>
                  <h3 className="la ih ph vh gi mj">
                    List Them For Sale
                  </h3>
                  <p className="eh oh uh">
                    Lorem ipsum dolor sit amet consectetur smit.
                  </p>
                </div>
              </div>
            </div>
          </div>
        </section>
        <section className="rg lg">
          <div className="a">
            <div className="ra td vd _d">
              <div className="ad qj">
                <h2 className="ia mh ph sh vh">
                  Popular Collection
                </h2>
                <a href="javascript:void(0)" className="ka _a yc _c ld he uf wf eh nh vh ii aj">
                  Explore More
                </a>
              </div>
            </div>
            <div className="ea za xc">
              <div className="pb nf _k/2">
                <div className="ja nd qd wd fe lf ik gl im">
                  <div className="ga za xc">
                    <div className="xb/3 qf">
                      <div className="oa pb">
                        <img src="src/images/collections/image-01.svg" alt="collection" className="pb" />
                      </div>
                    </div>
                    <div className="yb/3 qf">
                      <div className="oa pb">
                        <img src="src/images/collections/image-02.svg" alt="collection" className="pb" />
                      </div>
                      <div className="oa pb">
                        <img src="src/images/collections/image-03.svg" alt="collection" className="pb" />
                      </div>
                    </div>
                  </div>
                  <div className="sa za yc ad">
                    <h3>
                      <a href="javascript:void(0)" className="hh ph vh dk mk ul pm">
                        Creative 3d illustration
                      </a>
                    </h3>
                    <button className="_a yc ld ee _f ag">
                      <svg width={14} height={14} viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M6.99999 11.8709L6.15416 11.1009C3.14999 8.37675 1.16666 6.57425 1.16666 4.37508C1.16666 2.57258 2.57832 1.16675 4.37499 1.16675C5.38999 1.16675 6.36416 1.63925 6.99999 2.38008C7.63582 1.63925 8.60999 1.16675 9.62499 1.16675C11.4217 1.16675 12.8333 2.57258 12.8333 4.37508C12.8333 6.57425 10.85 8.37675 7.84582 11.1009L6.99999 11.8709Z" fill="#F85486" />
                      </svg>
                      <span className="tg kh nh wh">
                        1.6K
                      </span>
                    </button>
                  </div>
                  <div className="za yc">
                    <div className="ua kb pb ic gd ld">
                      <img src="src/images/collections/creator-01.jpg" alt="" className="ib pb af bf" />
                    </div>
                    <div className="pb">
                      <h4 className="eh nh vh">
                        @Lathium_Lui...
                        <span className="xa fh oh uh">
                          Creator
                        </span>
                      </h4>
                    </div>
                  </div>
                </div>
              </div>
              <div className="pb nf _k/2">
                <div className="ja nd qd wd fe lf ik gl im">
                  <div className="ga za xc">
                    <div className="yb/3 qf">
                      <div className="oa pb">
                        <img src="src/images/collections/image-04.svg" alt="collection" className="pb" />
                      </div>
                    </div>
                    <div className="xb/3 qf">
                      <div className="ga za xc">
                        <div className="yb/2 qf">
                          <div className="oa pb">
                            <img src="src/images/collections/image-05.svg" alt="collection" className="pb" />
                          </div>
                        </div>
                        <div className="yb/2 qf">
                          <div className="oa pb">
                            <img src="src/images/collections/image-06.svg" alt="collection" className="pb" />
                          </div>
                        </div>
                        <div className="pb qf">
                          <div className="oa pb">
                            <img src="src/images/collections/image-07.svg" alt="collection" className="pb" />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="sa za yc ad">
                    <h3>
                      <a href="javascript:void(0)" className="hh ph vh dk mk ul pm">
                        Modern illustration
                      </a>
                    </h3>
                    <button className="_a yc ld ee _f ag">
                      <svg width={14} height={14} viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M6.99999 11.8709L6.15416 11.1009C3.14999 8.37675 1.16666 6.57425 1.16666 4.37508C1.16666 2.57258 2.57832 1.16675 4.37499 1.16675C5.38999 1.16675 6.36416 1.63925 6.99999 2.38008C7.63582 1.63925 8.60999 1.16675 9.62499 1.16675C11.4217 1.16675 12.8333 2.57258 12.8333 4.37508C12.8333 6.57425 10.85 8.37675 7.84582 11.1009L6.99999 11.8709Z" fill="#F85486" />
                      </svg>
                      <span className="tg kh nh wh">
                        3.6K
                      </span>
                    </button>
                  </div>
                  <div className="za yc">
                    <div className="ua kb pb ic gd ld">
                      <img src="src/images/collections/creator-02.jpg" alt="" className="ib pb af bf" />
                    </div>
                    <div className="pb">
                      <h4 className="eh nh vh">
                        @Andrio_Hev...
                        <span className="xa fh oh uh">
                          Creator
                        </span>
                      </h4>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </section>
    
</div></Animatedpage>
  )
}
export default NftMarketplace